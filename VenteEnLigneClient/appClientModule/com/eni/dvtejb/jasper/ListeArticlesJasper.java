...
public class ListeArticlesJasper {
	
	public static void main(String[] args) 
	{		
		// Compilation
		String resultat = 
			compileReport("K:/ENI/DeveloppementEJB3/workspaceGalileo/VenteEnLigneClient/appClientModule/com/eni/dvtejb/jasper/",
					"ListeArticlesRapport");

		try {
			long start = System.currentTimeMillis();
			
				// création de l'entity manager factory pour la connexion avec la base de données
				EntityManagerFactory emf = Persistence.createEntityManagerFactory("VenteEnLigneClientJavaSE", new HashMap());
				EntityManager em = emf.createEntityManager();
				
				try
				{
					Map parameters = getParameters(em);					
					JasperRunManager.runReportToPdfFile("K:/ENI/DeveloppementEJB3/workspaceGalileo/VenteEnLigneClient/appClientModule/com/eni/dvtejb/jasper/ListeArticlesRapport.jasper", parameters);					
					em.close();
	...
	private static Map getParameters(EntityManager em) {
		...
		parameters.put("TitreRapport", "Liste des articles");
		return parameters;
	}
